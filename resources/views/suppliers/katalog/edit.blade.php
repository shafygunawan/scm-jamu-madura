<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Edit Entri Katalog</h2>
        ...
        <div class="mx-auto max-w-2xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('suppliers.katalog.update', $catalog) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="supplier_id" required
                                ...
                                @foreach ($suppliers as $s)
                                    <option value="{{ $s->id }}" @selected($catalog->supplier_id == $s->id)>{{ $s->nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="raw_material_id" required
                                ...
                                @foreach ($rawMaterials as $r)
                                    <option value="{{ $r->id }}" @selected($catalog->raw_material_id == $r->id)>{{ $r->nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Harga (per
                                unit)</label>
                            <input type="number" name="harga" step="0.01" required value="{{ $catalog->harga }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
                        </div>
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Lead Time
                                (hari)</label>
                            <input type="number" name="lead_time" min="0" value="{{ $catalog->lead_time }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" />
                        </div>
                        ...
                        <div class="flex justify-end gap-3">
                            ...
                            <form action="{{ route('suppliers.katalog.destroy', $catalog) }}" method="POST"
                                onsubmit="return confirm('Hapus entri ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="inline-block rounded-md bg-red-600 px-4 py-2 text-white hover:bg-red-700">Hapus</button>
                            </form>
                        </div>
                    </form>
                </div>
